...
class TestWorkFlow(unittest.TestCase):
    ...
    def test_sequential_api(self):
        ...
        sequential.add(Dense(4, input_dim=20, activation="relu"))
        sequential.add(Dense(2, input_dim=4))
        sequential.add(Activation("relu"))
        sequential.compile(optimizer="Adagrad", loss="categorical_crossentropy")
        # generate dummy data
        import numpy as np
        data = np.random.random((100, 20))
        labels = np.random.randint(low=1, high=2, size=(100, 1))
        sequential.fit(x=data, y=labels, batch_size=8, nb_epoch=10)
