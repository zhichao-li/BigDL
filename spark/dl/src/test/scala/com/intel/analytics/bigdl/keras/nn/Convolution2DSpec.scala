  ...
  "Convolution2D NCHW" should "be the same as Keras" in {
    val kerasCode =
      """
        ...
        |output_tensor = Convolution2D(64, 2, 5, activation="relu", dim_ordering="th")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = Convolution2D[Float](64, 2, 5, activation = ReLU(), inputShape = Array(3, 24, 24))
    seq.add(layer)
  ...
  "Convolution2D NHWC" should "be the same as Keras" in {
    val kerasCode =
      """
        ...
        |output_tensor = Convolution2D(32, 4, 6, border_mode="same", dim_ordering="tf")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = Convolution2D[Float](32, 4, 6, format = DataFormat.NHWC,
      borderMode = "same", inputShape = Array(24, 24, 3))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
      kerasCode, weightConverter, 1e-3)
  }

  "Convolution2D without bias" should "be the same as Keras" in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[3, 24, 24])
        |input = np.random.random([2, 3, 24, 24])
        |output_tensor = Convolution2D(64, 2, 5, bias=False, subsample=(2, 3), dim_ordering="th")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = Convolution2D[Float](64, 2, 5, subsample = (2, 3),
      bias = false, inputShape = Array(3, 24, 24))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
