...
class MaxPooling2DSpec extends KerasBaseSpec{
  ...
  "MaxPooling2D NCHW" should "be the same as Keras" in {
    ...
    val layer = MaxPooling2D[Float](inputShape = Array(3, 24, 24))
    seq.add(layer)
  ...
  "MaxPooling2D NHWC" should "be the same as Keras" in {
    val kerasCode =
      """
        ...
        |output_tensor = MaxPooling2D(pool_size=(2, 3), strides=(1, 2), dim_ordering="tf")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = MaxPooling2D[Float](poolSize = (2, 3), strides = (1, 2),
      format = DataFormat.NHWC, inputShape = Array(32, 28, 5))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
      kerasCode)
  }
